...
void threadRemoval(Thread aThread) {
	// set head of the list to the next node
	if (aThread->tid == head->tid) {
		//if there are no more items left in the list
	...
	// delete references to this thread
	aThread->next = NULL;
	aThread->prev = NULL;
}
...
// function for testing if all jobs are complete. 1 indicates all done, 0 indicates not done
...
// runs all threads, can be modified to run next thread I think
void scheduler() {
