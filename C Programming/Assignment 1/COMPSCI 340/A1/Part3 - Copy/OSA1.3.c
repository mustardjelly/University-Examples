/*
 ...
 UPI         : mgu011
 Version     : 1.0
 Description : Thread implementation. (P3)
 ============================================================================
 */
...
#define INTERVAL 20000 // microseconds in 20 milliseconds

Thread newThread; // the thread currently being set up
Thread mainThread; // the main thread
Thread head; // the head of the linked list
...
struct itimerval timer_val; // information about a timer interval
...
// forces the currently executing thread to suspend and switch to
// next available thread
void threadYield() {
	scheduler();
}

// defines information for the internal timer
// example given at:
// http://forums.codeguru.com/showthread.php?356101-How-to-use-timer-in-Unix
void setTimer() {
	timer_val.it_interval.tv_sec = 0;
	timer_val.it_interval.tv_usec = INTERVAL; // sets interval to 20 milliseconds
	timer_val.it_value.tv_sec = 0;
	timer_val.it_value.tv_usec = INTERVAL; // sets initial value to 20 milliseconds
	setitimer(ITIMER_VIRTUAL, &timer_val, &timer_val); // type of timer, old val, new val
	
	signal(SIGVTALRM, threadYield); // catch SIGVTALRM and run threadYield;
}
